...
<template>
  <div class="max-w-4xl mx-auto px-4 py-12">
    ...
    <div v-else class="grid gap-8">
      <article v-for="post in posts" :key="post.id" class="group relative flex flex-col items-start">
         <div v-if="post.coverImage" class="w-full h-64 mb-4 overflow-hidden rounded-xl bg-zinc-100 dark:bg-zinc-800">
             <img :src="post.coverImage" :alt="post.title" class="object-cover w-full h-full group-hover:scale-105 transition-transform duration-300" />
         </div>
        <h2 class="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
